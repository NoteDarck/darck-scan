...
const publishManga = async () => {
  ...
  try {
    ...
    const publishedMangas = JSON.parse(localStorage.getItem('publishedMangas') || '[]');
    const newManga = {
      id: Date.now(),
      ...mangaData.value,
    ...
    publishedMangas.push(newManga);
    localStorage.setItem('publishedMangas', JSON.stringify(publishedMangas));
